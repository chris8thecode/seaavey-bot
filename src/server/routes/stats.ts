...
export const statsRoutes = new Elysia({ prefix: "/api/stats" })
  .get("/", () => {
    const totalUsers = (db.query("SELECT COUNT(*) as count FROM users").get() as { count: number })
      .count;
    const totalGroups = (
      db.query("SELECT COUNT(*) as count FROM groups").get() as { count: number }
    ).count;
    ...
    const commandsToday =
      (
        db
          .query("SELECT SUM(hits) as total FROM users WHERE lastChat >= ?")
          .get(today.getTime()) as { total: number | null }
      ).total || 0;
    ...
    return {
      ...
      commandsToday,
      messagesProcessed:
        (db.query("SELECT SUM(hits) as total FROM users").get() as { total: number | null })
          .total || 0,
    };
  })
  .get("/chart", () => {
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      const row = db
        .query("SELECT COUNT(*) as count FROM group_members WHERE lastChat >= ? AND lastChat < ?")
