...
import db, { setGroup, type Group } from "@/infra/database"
...
export const groupsRoutes = new Elysia({ prefix: "/api/groups" })
  .get("/", () => {
    ...
    return rows.map((g) => ({
      ...
      members: (db.query("SELECT COUNT(*) as count FROM group_members WHERE groupJid = ?").get(g.jid) as { count: number }).count,
  ...
  .patch("/:jid", ({ params, body }) => {
    ...
    if (data.antiviewonce !== undefined) setGroup(params.jid, "antiviewonce", data.antiviewonce ? 1 : 0)
    if (data.autosticker !== undefined) setGroup(params.jid, "autosticker", data.autosticker ? 1 : 0)
